...
while read NAME;do cat $databasepath/database/hallmakrpathway/$NAME.txt $1|sort |uniq -d |xargs -n9000|sed 's/ /,/g'; done < $databasepath/database/hallmakrpathway/name >tmp1
...
paste  tmp2 tmp3 $databasepath/database/hallmakrpathway/number tmp4 tmp1|awk 'BEGIN{FS="\t"}{if(!$5){print $0"null";next}print $0}' >tmp5
...
while read NAME;do cat $databasepath/database/hallmakrpathway/hallmark/$NAME $1|sort |uniq -d |xargs -n5000|sed 's/ /,/g'; done < $databasepath/database/hallmakrpathway/hallmark/file >tmp1
...
sed 1d 0|sed 's/\"//g;s/ /\t/g'|cut -f 2 >tmp6
#
paste $databasepath/database/hallmakrpathway/hallmark/name $databasepath/database/hallmakrpathway/hallmark/name2 tmp5 tmp6 |awk 'BEGIN{FS="\t"}{print $0"\t"($3/$4)/($5/$6)}'|sed 's/\t/#/g' >sub_pathways.enrich.xls
sed -i '1i Main Cancer hall mark pathway#hallmark#hallmark pathway#my proteins in this hallmark#my proteins in all hallmark#genes in this hallmark#genes in all hallmark#gene list#p.value#enrichment ratio' sub_pathways.enrich.xls
#
##rm tmp*
echo "Name p1 p2" |sed 's/ /\t/g' >data.txt 
sed 's/#/\t/g' sub_pathways.enrich.xls|cut -f 3,9  |sed 1d|awk 'BEGIN{FS="\t"}{a=$1;b=$2;if($2==0){b=1e-10}print a"\t"b}' |awk 'BEGIN{FS="\t"}{print $1"\t0.001\t"(-1)*log($2)/log(10)}' >>data.txt
#awk 'BEGIN{FS="\t"}{a=$3;b=$4;if($3==0){a=0.0000000001}if($4==0){b=0.000000001}print $1"\t"$2"\t"a"\t"b}' data.txt >m
##mv m data.txt
...
nba=data 
CairoJPEG(\"wheel.jpg\",width=1200,height=1200) 
...
p2 = ggplot(nba.m, aes(x=Name, y=var2, fill=value)) +
     ...
     geom_text(data=nba.labs, aes(x=Name, y=var2+0.6,
        label=\"\", angle=ang, hjust=hjust), size=4) +
...
dev.off() " >codewheel.R
Rscript codewheel.R
python $databasepath/src/merge.wheel.plot.py $path
rm tmp* 0

cut -d "#" -f 1,2,4- sub_pathways.enrich.xls >0
mv 0 sub_pathways.enrich.xls
sed -i 's/#/\t/g' *.xls
